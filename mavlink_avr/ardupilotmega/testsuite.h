...
static void mavlink_test_ap_engine_status(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
	mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_AP_ENGINE_STATUS >= 256) {
        	return;
        }
#endif
	mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
	mavlink_ap_engine_status_t packet_in = {
		17.0,45.0,73.0,17859
    };
	mavlink_ap_engine_status_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        packet1.tmp = packet_in.tmp;
        packet1.ffr = packet_in.ffr;
        packet1.frm = packet_in.frm;
        ...
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_AP_ENGINE_STATUS_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_AP_ENGINE_STATUS_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_ap_engine_status_encode(system_id, component_id, &msg, &packet1);
	mavlink_msg_ap_engine_status_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_ap_engine_status_pack(system_id, component_id, &msg , packet1.rpm , packet1.tmp , packet1.ffr , packet1.frm );
	mavlink_msg_ap_engine_status_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_ap_engine_status_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.rpm , packet1.tmp , packet1.ffr , packet1.frm );
	mavlink_msg_ap_engine_status_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
	...
	mavlink_msg_ap_engine_status_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_ap_engine_status_send(MAVLINK_COMM_1 , packet1.rpm , packet1.tmp , packet1.ffr , packet1.frm );
	mavlink_msg_ap_engine_status_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
...
static void mavlink_test_ardupilotmega(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
	...
	mavlink_test_ap_engine_status(system_id, component_id, last_msg);
